...
void CRoomTemplates::PrintTemplates() {
  ...
  for (int i = 0; i < int(m_rooms.size()); i++) {
    std::vector<v2i> vertices = m_rooms[i].GetVertices();
...
bool CRoomTemplates::LoadTemplatesFromXML(const char *fileName) {
  ...
  while (xmlNode != 0) {
    if (strcmp(xmlNode->Value(), "Room") == 0) {
      ...
      // Parse a room...
      CRoom room;
      std::vector<v2i> vertices;
      TiXmlNode *xmlChildNode = xmlNode->FirstChild();
      while (xmlChildNode != 0) {
        if (strcmp(xmlChildNode->Value(), "Vertex") == 0) {
          // Parse a vertex...
          int px, py;
          int rx = xmlChildNode->ToElement()->QueryIntAttribute("px", &px);
          int ry = xmlChildNode->ToElement()->QueryIntAttribute("py", &py);
          ...
          int idx;
          int r =
              xmlChildNode->ToElement()->QueryIntAttribute("edgeIndex", &idx);
          if (r == TIXML_SUCCESS) {
...
bool CRoomTemplates::SaveTemplatesAsXML(const char *fileName) {
  ...
  for (int i = 0; i < GetNumOfTemplates(); i++) {
    ...
    v2i shift = GetRoom(i).GetCenterShift();
    std::ostringstream oss0;
    std::ostringstream oss1;
    oss0 << shift[0];
    oss1 << shift[1];
