...
bool CLevelConfig::LoadFromSynConfig(std::string fileName, unsigned seed,
  ...
  if (fin.fail() == true) {
    std::cout << "Failed to load config parameters from config file "
              << fileName << "!\n";
    return false;
  }
  std::string param;
...
void CLevelConfig::DumpTimeAndDate(FILE *file) {
  ...
  fprintf(file, "%02d:%02d:%02d ", ptrTime->tm_hour, ptrTime->tm_min,
          ptrTime->tm_sec);
...
void CLevelConfig::DumpTimeAndDate(std::ofstream &fout) {
  ...
  tm *ptrTime = localtime(&myTime);
  char str[1000];
  sprintf(str, "%02d:%02d:%02d ", ptrTime->tm_hour, ptrTime->tm_min,
          ptrTime->tm_sec);
  fout << str;
  sprintf(str, "%02d/%02d/%04d", ptrTime->tm_mon + 1, ptrTime->tm_mday,
          ptrTime->tm_year + 1900);
  fout << str << std::endl;
}
...
void CLevelConfig::DumpParameters(FILE *file) {
  ...
  fprintf(file, "%s\t%d\n", "FLAG_DISCRETE_CONNECTIVITY_FUNCTION",
          m_flagDiscreteConnectFunc);
  fprintf(file, "%s\t%d\n", "FLAG_RANDOM_PICK", m_flagRandomPick);
  fprintf(file, "%s\t%d\n", "FLAG_NON_OVERLAP_CONTACT",
          m_flagNonOverlapContact);
  fprintf(file, "%s\t%d\n", "FLAG_SMALL_FACE_FIRST", m_flagSmallFaceFirst);
  fprintf(file, "%s\t%d\n", "FLAG_USE_ILS", m_flagUseILS);
  fprintf(file, "%s\t%d\n", "FLAG_RANDOM_WALK", m_flagRandomWalk);
  fprintf(file, "%s\t%d\n", "NUMBER_OF_SOLUTIONS_TO_TRACK",
          m_numOfSolutionsToTrack);
...
void CLevelConfig::UpdateOutputPrefix() {
...
#else
  while (m_outputPrefix.empty() == false &&
         isdigit(m_outputPrefix[m_outputPrefix.size() - 1]) == 0) {
  ...
  while (flag == false && m_outputPrefix.size() >= 2) {
    std::string subStr =
        m_outputPrefix.substr(m_outputPrefix.length() - numLength - 1, 2);
    int num = atoi(subStr.c_str()) + 1;
    char numChar[MAX_PATH];
    sprintf_s(numChar, "%02d", num);
    std::ostringstream oss;
    oss << m_outputPrefix.substr(0, m_outputPrefix.length() - numLength - 1)
        << numChar << "\\";
  ...
  std::string outputFolder =
      m_outputPrefix.substr(0, m_outputPrefix.size() - 1);
...
#endif
  std::cout << "Generating results into the directory: "
            << m_outputPrefix.c_str() << "...\n";
}

void CLevelConfig::DumpStringParam(FILE *file, const char *param,
