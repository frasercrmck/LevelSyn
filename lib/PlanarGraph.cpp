...
void CPlanarGraph::PrintGraph() {
  std::cout << "The graph contains " << m_nodes.size() << " nodes and "
            << m_edges.size() << " edges!\n";
...
bool CPlanarGraph::LoadGraphFromXML(const char *fileName,
                                    bool flagDetectFaces /* = true */,
...
bool CPlanarGraph::CheckDuplicatedEdge(const CGraphEdge &edge) {
  ...
  for (int i = 0; i < GetNumOfEdges(); i++) {
    ...
    if ((idx00 == idx10 && idx01 == idx11) ||
        (idx00 == idx11 && idx01 == idx10)) {
      return true;
    }
  }
  return false;
...
void CPlanarGraph::MovePickedNode(int dx, int dy) {
  if (m_pickedNodeIndex < 0) {
    return;
  }
  v2i pos = m_nodes[m_pickedNodeIndex].GetPos();
  pos[0] += dx;
  pos[1] += dy;
...
void CPlanarGraph::DetectFaces() {
  ...
  // (http://www.boost.org/doc/libs/1_47_0/libs/graph/example/planar_face_traversal.cpp)
  ...
  for (boost::tie(vi, vi_end) = boost::vertices(g); vi != vi_end; ++vi) {
    ...
    for (boost::tie(ei, ei_end) = boost::out_edges(*vi, g); ei != ei_end;
         ++ei) {
...
void CPlanarGraph::SortAdjacentVertices(
    const Graph &g, VertexDescriptor vert,
    std::vector<EdgeDescriptor> &adjacentEdges) {
  ...
  v2i edgeRef = GetNode(v_0).GetPos() - GetNode(vert).GetPos();
  for (int i = 0; i < adjacentEdges.size(); ++i) {
    for (int j = i + 1; j < adjacentEdges.size(); ++j) {
      ...
      v2i edgePr1 = GetNode(v_i).GetPos() - GetNode(vert).GetPos();
      v2i edgePr2 = GetNode(v_j).GetPos() - GetNode(vert).GetPos();
      if (CompareEdgeDirections(edgePr1, edgePr2, edgeRef)) {
...
bool CPlanarGraph::CompareEdgeDirections(const v2i &edgePr1, const v2i &edgePr2,
                                         const v2i &edgeRef) {
...
#if 0
  std::cout << "To remove the " << maxFaceIdx << "th face of size "
            << m_faces[maxFaceIdx].GetFaceSize() << "...\n";
#endif
...
std::vector<int>
CPlanarGraph::ExtractDeepestFaceOrChain(bool &flagCyclic,
  ...
  if (faceIndices.empty() == false &&
      faceConstraintCount > chainConstraintCount) {
    flagCyclic = true;
    return faceIndices;
  } else {
    flagCyclic = false;
...
std::vector<int>
CPlanarGraph::ExtractDeepestFaceOrChainOld(bool &flagCyclic,
                                           bool flagSmallFaceFirst) {
  std::vector<int> faceIndices = ExtractDeepestFace(flagSmallFaceFirst);
  if (faceIndices.empty() == false) {
    flagCyclic = true;
...
std::vector<int> CPlanarGraph::ExtractDeepestChainNew() {
  ...
  for (int i = 0; i < GetNumOfNodes(); i++) {
    ...
    int chainConstraintCount = CountConstraints(chainIndices);
    if (chainConstraintCount >= chainLengthMinC &&
        chainLengthTmp < chainLengthMin) {
      chainLengthMinC = chainConstraintCount;
      chainLengthMin = chainLengthTmp;
      chainMin = chainIndices;
    }
    if (chainConstraintCount >= chainLengthMaxC &&
        chainLengthTmp > chainLengthMax) {
...
void CPlanarGraph::GetGraphBoundingBox(v2i &posMin, v2i &posMax) {
  v2i pMin(std::numeric_limits<int>::max());
  v2i pMax(std::numeric_limits<int>::min());
  for (int i = 0; i < GetNumOfNodes(); i++) {
    v2i pi = GetNode(i).GetPos();
    for (int j = 0; j < 2; j++) {
...
void CPlanarGraph::MoveGraphToSceneCenter() {
  ...
  v2i posCen = floor_midpoint(posMin, posMax);
  for (int i = 0; i < GetNumOfNodes(); i++) {
    v2i pi = GetNode(i).GetPos();
    pi = pi - posCen;
    GetNode(i).SetPos(pi);
