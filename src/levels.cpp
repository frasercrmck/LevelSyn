...
void usage(char *argv[]) {
  std::cout << "Usage: " << argv[0]
            << " graph.xml templates.xml config.txt [target_solution_number]\n";
}
...
int main(int argc, char **argv) {
  ...
  planar_graph.SaveGraphAsXML(
      CLevelConfig::AddOutputPrefix(input_graph_name).c_str());
  ...
  fout << "Have found " << level_synthesizer.GetSolutionCount()
       << " solution(s) within " << elapseTime << " seconds.\n";
  std::cout << "Have found " << level_synthesizer.GetSolutionCount()
            << " solution(s) within " << elapseTime << " seconds.\n";
